...
InstallRedis(){
  ...
  if [ -f "/tmp/$PACKAGE_NAME" ]; then
    ...
    if [ $CHECKSUM == $DOWNLOAD_CHECKSUM ]; then
      ...
      chmod 777 /var/log/redis/redis-sentinel.log
...
configure(){
  case "$OS_RELEASE" in
   ubuntu)
    if [ -f /tmp/redis-server.override ]; then
      echo "Moving Redis Sentinel service override into place..."
      ...
      echo "Redis Sentinel service override file not found at /tmp/redis-server.override"
      exit 1
    fi

    if [ -f /tmp/redis-server.conf ]; then
      echo "Moving Redis Sentinel service job file into place..."
      mv /tmp/redis-server.conf /etc/init/
      chown root.root /etc/init/redis-server.conf
      chmod 644 /etc/init/redis-server.conf
    else
      echo "Redis Sentinel service job file not found at /tmp/redis-server.conf"
      exit 1
    fi
   ...
   centos|rhel|ol)
    if [ -f /tmp/redis-server.conf ]; then
      echo "Moving Redis Sentinel service job file into place..."
      mv /tmp/redis-server.conf /etc/init.d/redis-server.conf
  ...
  if [ -f /tmp/sentinel.conf ]; then
    echo "Copying Redis Sentinel configuration into place..."
    cp /tmp/sentinel.conf /etc/redis/sentinel.conf
    chown root:root /etc/redis/sentinel.conf
    chmod 777 /etc/redis/sentinel.conf
  else
    echo "Redis Server configuration file not found at /tmp/sentinel.conf"
    exit 1
  fi
